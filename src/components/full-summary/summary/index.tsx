...
const Summary = ({ data, isLoading }: Props) => {
  ...
  return (
    <>
      <section className="mx-auto xl:ml-3 w-full max-w-[580px] lg:max-w-[770px] flex flex-col gap-10 md:gap-[62px] px-4 xl:px-0">
        <section className="w-full flex items-start gap-5 flex-col lg:flex-row relative">
          <div className="mt-2 lg:absolute lg:-left-[80px]">
            {isLoading ? (
          ...
          <Stack spacing={14} className="w-full">
            ...
            <div className="flex items-center gap-2 lg:gap-3 text-navy-300 text-xs lg:text-base text-center">
              ...
              {isLoading ? (
                ...
                <p className="text-xs lg:text-sm font-[500] leading-[21.8px] uppercase">
                  {date}
                </p>
              )}
            </div>
              ...
              {isLoading ? (
                ...
                <>
                  <p className="font-[500] text-[11px] leading-[21.8px] text-navy-300 italic align-middle uppercase">
                    ORIGINALLY POSTED BY @{data?.author} ON YOUTUBE. ALL RIGHTS
